  ...
  try {
    ...
    try {
      ...
      if (!user.id || !user.email) {
        console.error("[v0] User data incomplete - id:", user.id, "email:", user.email)
        return NextResponse.json({ success: false, error: "User data incomplete" }, { status: 500 })
      }

      console.log("[v0] Creating JWT with jose library...")
      console.log("[v0] Creating JWT with userId:", user.id, "email:", user.email)
